...
main = do
  ...
  putStrLn "\n1. Computing 2 + 3:"
  displayResults (map fst $ runDFS (cons (s (s z)) (s (s (s z)))) additionEx2 :: [SExp Int])
  
  putStrLn "\n2. What numbers add up to 5? (using dual relation):"
  displayResults (map fst $ runDFS (s (s (s (s (s z))))) (dual additionEx2) :: [SExp Int])
  
  putStrLn "\n3. Addition using polytypic hylomorphism (2 + 3):"
  displayResults (map fst $ runDFS (cons (s (s z)) (s (s (s z)))) additionEx3 :: [SExp Int])

  putStrLn "\n4. What numbers add up to 5 using polytypic hylomorphism?:"
  displayResults (map fst $ runDFS (s (s (s (s (s z))))) (dual additionEx3) :: [SExp Int])
